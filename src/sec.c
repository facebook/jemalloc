...
static sec_bin_t *
sec_bin_pick(tsdn_t *tsdn, sec_t *sec, pszind_t pszind) {
	uint8_t shard = sec_shard_pick(tsdn, sec);
	size_t  ind = pszind * sec->opts.nshards + shard;
...
sec_fill(tsdn_t *tsdn, sec_t *sec, size_t size, edata_list_active_t *result,
	...
	sec_bin_t *bin = sec_bin_pick(tsdn, sec, pszind);
